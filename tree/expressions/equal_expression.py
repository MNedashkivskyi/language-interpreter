...
class EqualExpression(Expression):
    ...
    def __str__(self):
        ...
        if len(self.expressions) == 0:
            equal_operator_str = "_"
        else:
            if len(self.expressions) > 1:
                equal_operator_str += "("
            equal_operator_str += f'{self.expressions[0]}'

            for i in range(1, len(self.expressions)):
                equal_operator_str += f" {self.operators[i - 1]}"
                equal_operator_str += f' {self.expressions[i]}'

            if len(self.expressions) > 1:
                equal_operator_str += ")"

        return equal_operator_str

...
class RelationExpression(Expression):
    ...
    def __str__(self):
        relation_operator_str = ""
        if len(self.expressions) == 0:
            relation_operator_str = "_"
        else:
            if len(self.expressions) > 1:
                relation_operator_str += "("
            relation_operator_str += f'{self.expressions[0]}'

            for i in range(1, len(self.expressions)):
                relation_operator_str += f" {self.operators[i - 1]}"
                relation_operator_str += f' {self.expressions[i]}'

            if len(self.expressions) > 1:
                relation_operator_str += ")"

        return relation_operator_str
